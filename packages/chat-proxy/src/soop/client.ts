...
import { BaseChatClient, type ChatClientOptions, type Platform } from '../chat-types'
import { ChatConnectionError } from '../errors'
import { fetchSoopChatConnection } from './api'
...
/**
 * SOOP 비공식 채팅 WebSocket 클라이언트. 읽기 전용, 항상 익명으로 연결합니다.
 *
...
export class SoopChatClient extends BaseChatClient {
  ...
  async connect(): Promise<void> {
    ...
    const session = await fetchSoopChatConnection(this.channelId, {
      domain: this.domain,
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const settleErr = (error: Error) => {
        if (settled) return
        settled = true
        reject(new ChatConnectionError(`SOOP 채팅 연결 실패: ${error.message}`, { cause: error }))
      }

      ws.on('open', () => {
        this.sendPacket(SVC.CONNECT, connectPayload())
      })
